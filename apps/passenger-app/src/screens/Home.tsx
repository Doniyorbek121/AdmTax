      ...
      <div className="relative z-10 mt-auto sheet">
        <div className="bg-white rounded-t-3xl shadow-2xl p-5 pb-8">
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-ink-900">{t('Qayerga boramiz?')}</h2>
            {cars.length > 0 && (
              <span className="flex items-center gap-1.5 text-xs font-semibold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" /> {cars.length} {t('mashina yaqinda')}
              </span>
            )}
          </div>
          ...
          >
            ...
            <span className="text-ink-500 font-medium">{t('Manzilni kiriting…')}</span>
          </button>
